...
### Fixed
- **Plan Visibility**: Resolved issue where plans disappeared after editing due to type mismatch (boolean vs integer) in `is_active` status handling.
- **Admin Panel**: Fixed "Active" checkbox in Plan Edit modal to correctly reflect status from PostgreSQL.
- **Data Integrity**: Reset all existing plans to `is_active = TRUE` to restore visibility.
